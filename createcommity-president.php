...
if (isset($_POST['submit'])) {
    if (isset($_POST['commity_president'])) {
        $selectedId = $_POST['commity_president']; // Get the ID of the selected president
        ...
        if ($userResult->num_rows > 0) {
            ...
            // Insert the selected president into the president table
            $insertQuery = "INSERT INTO president (id, flat, name, number, email) VALUES (?, ?, ?, ?, ?)";
            $insertStmt = $conn->prepare($insertQuery);
            $insertStmt->bind_param("issss", $user['id'], $user['flat'], $user['name'], $user['number'], $user['email']);

            if ($insertStmt->execute()) {
                $_SESSION['selected_president'] = $user['id']; // Store selected president ID
                $successMessage = "President selected and added successfully.";
                header("Location: createcommity-secretory.php");
                exit();
            } else {
                $errorMessage = "Failed to add the president.";
            }
            $insertStmt->close();
        } else {
...
<body>
    ...
    <div class="main-content">
        ...
        <?php endif; ?>
        <div class="select ```php"  style="display:flex;">
